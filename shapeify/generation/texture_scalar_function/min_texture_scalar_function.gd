...
func evaluate(texture: LocalTexture) -> float:
	...
	var min_channel: float = INF
	for i in range(workgroup_size_x):
		min_channel = min(min_channel, components[i])
	return min_channel
...
func _load_shader():
	...
	# Create our _shader.
	var shader_file := load("res://shaders/compute/texture_scalar_function/min_texture_scalar_function.glsl")
