...
func _weight_texture_set():
	...
	_weight_texture_set_rid = _create_texture_uniform_set(weight_texture.rd_rid, 4)
	_mpa_metric.weight_texture = weight_texture
	_global_value_invalidated = true
...
func _compute(subrect: Rect2i) -> float:
	...
	_rd.compute_list_bind_uniform_set(compute_list, _weight_texture_set_rid, 4)
	...
	for n in weights_partial_sums:
		total_weights += n
	var mpa = _global_metric_value + delta_mpa_sum / (total_weights * 3.0)
	return mpa
