...
func evaluate(texture: RendererTexture) -> float:
	...
	var max_channel: float = 0.0
	for i in range(workgroup_size_x):
		max_channel = max(max_channel, components[i])
	return max_channel
...
func _load_shader():
	...
	# Create our _shader.
	var shader_file := load("res://shaders/compute/texture_scalar_function/max_texture_scalar_function.glsl")
